"""Phase 5C dev visual check — Compare-priority Playwright sweep.

Opens 4 dev pages and asserts console_error == 0:
  1. /compare                                       (priority)
  2. /discovery
  3. /debug/report-v2?fixture=discovery-sample      (ui unset -> v2)
  4. /debug/report-v2?fixture=discovery-sample&ui=v1 (rollback path)

Exit 0 if all pages pass, 1 if any console error found.
"""
import json
import sys
...
PAGES = [
    ...
    ("debug_v2_default", f"{BASE}/debug/report-v2?fixture=discovery-sample"),
    ("debug_v2_v1_query", f"{BASE}/debug/report-v2?fixture=discovery-sample&ui=v1"),
]


def seed_auth(context):
    token = "phase5c-dev-stub"
    user = {"role": "admin", "display_name": "Phase 5C Tester"}
