...
const components = {
  h1: ({ children }) => {
    ...
    return <h2 id={id} className="text-2xl font-bold japanese-text text-on-surface scroll-mt-20">{children}</h2>
  },
  h2: ({ children }) => {
    const id = makeHeadingId(String(children))
    return <h3 id={id} className="text-xl font-bold japanese-text text-on-surface scroll-mt-20">{children}</h3>
  },
  h3: ({ children }) => {
    const id = makeHeadingId(String(children))
    return <h4 id={id} className="text-lg font-bold japanese-text text-on-surface scroll-mt-20">{children}</h4>
  },
  p: ({ children }) => (
    <p className="text-sm leading-7 whitespace-pre-wrap text-on-surface-variant japanese-text">{children}</p>
  ),
  ...
  code: ({ children, className }) => {
    ...
    return (
      <code className="px-1.5 py-0.5 rounded bg-surface-container text-xs">{children}</code>
    )
  },
  pre: ({ children }) => (
    <pre className="overflow-x-auto rounded-2xl bg-surface-container p-4 text-xs text-on-surface-variant">
      {children}
    </pre>
  ),
  blockquote: ({ children }) => (
    <blockquote className="border-l-4 border-secondary/60 bg-surface-container-lowest pl-4 pr-3 py-3 rounded-r-xl text-sm text-on-surface-variant italic">
      {children}
    </blockquote>
  ),
  ul: ({ children, depth }) => {
    ...
    return (
      <ul className={`space-y-1.5 ${indent} list-disc text-sm text-on-surface-variant`}>
        {children}
      </ul>
    )
  },
  ol: ({ children, depth }) => {
    const indent = depth > 0 ? 'pl-5' : 'pl-6'
    return (
      <ol className={`space-y-1.5 ${indent} list-decimal text-sm text-on-surface-variant`}>
        {children}
      </ol>
    )
  },
