...
const faqs = [
  {
    q: '基本レポートを使うために追加設定は必要ですか？',
    a: 'サイトの計測データを接続すれば利用できます。競合・入口ページ・クリエイティブの高度分析は、品質を確認しながら運用担当者が個別にご案内します。',
  },
  {
    q: 'Webサイトのデータを表示するには何が必要ですか？',
    a: 'サイトの計測データを安全に保存し、Insight Studioから読み取れる設定が必要です。導入前に、権限・取得状況・最新の計測日を一緒に確認します。',
  },
  {
    q: '料金はどのように決まりますか？',
    a: '現在は先行導入期間のため、接続するサイト数、分析範囲、初期設定支援、運用支援の有無を確認して提示します。追加費用が発生する機能は、利用開始前に対象と上限をお知らせします。',
  },
  {
    q: '接続したデータは安全に管理されますか？',
    a: '案件ごとに閲覧権限を分け、分析に必要な最小限の範囲だけを読み取ります。保存期間、削除申請、共有範囲は導入時に確認できます。',
  },
]
