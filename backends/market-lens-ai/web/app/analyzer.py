...
5. ## 実行プラン
   ...
   - ### 5-1 LP改善施策 [MUST]
   - ### 5-2 検索広告施策 [MUST]
   - ### 5-3 Meta/ディスプレイ施策 [OPTIONAL — token逼迫時のみ省略可]
   - ### 5-4 KPI測定計画 [OPTIONAL — token逼迫時のみ省略可]

token が逼迫したら Section 4 と 5-3 / 5-4 を圧縮してよい。
**ただし最優先3施策 / 5-0 / 5-1 / 5-2 は死守すること（省略・短縮禁止）。**
...
### 5. 実行プラン
以下の**6サブセクションに分離**して出力。
**Section 5 は絶対に途中で終わらせない。token不足時は Section 4 を圧縮して Section 5 を完結させること（Task E）。**

#### 最優先3施策 [MUST]
- 全レイヤー横断の最優先3件を冒頭に列挙（必ず `### 最優先3施策` 見出しをつける）
- 各施策は `施策名 / 対象ブランド / 期待効果 / 優先度(S/A/B)` の4点を1行で
- 省略禁止。token逼迫でもこのブロックは削らないこと

#### 5-0. 予算フレーム [MUST]
- `### 5-0 予算フレーム` 見出しをつけ、テーブル形式で出力
- 列: `| 項目 | 初期フェーズ | 拡張フェーズ | 備考 |`
- 月額予算帯・CPA ガイドライン・想定 CV を必ず含める（推定でよい）
- 根拠は市場推定ブロック（上の市場概況テーブル）の数値を必ず参照
- 省略禁止
...
#### 5-2. 検索広告施策（Task F: 運用粒度強化） [MUST]
`| ブランド | 獲得タイプ | 優先度 | アクション | 推奨訴求 | 推奨着地先 | 主戦場クエリ | 除外クエリ例 | 初回KPI |`
- 獲得タイプは必ず3分類: **指名防衛** / **カテゴリ非指名** / **比較検討**
- 各行に推奨クエリ例（3つ以上）、除外クエリ例（2つ以上）、着地先、主KPIをセットで出す
- **5-2 は絶対に未完で終わらせない。テーブルは最後の行まで閉じること**

#### 5-3. Meta / ディスプレイ施策 [OPTIONAL]
`| ブランド | 獲得タイプ | 優先度 | アクション | 推奨訴求 | 推奨着地先 | 初回KPI |`
- token不足の場合のみ `（token制約により省略）` として5-2の完結を優先してよい

#### 5-4. KPIフレーム・測定計画 [OPTIONAL]
KPI測定計画テーブルを出力（フォーマットは {_KPI_FRAMEWORK_LAYER} の指示に従う）。
- token逼迫時は5-4より5-2の完結を優先してよい
...
#### 出力完結ルール（Task E 必須）:
- `最優先3施策` → `5-0. 予算フレーム` → `5-1. LP改善` → `5-2. 検索広告` の順で死守
- 上記4サブセクションは token 逼迫時も省略禁止。省略できるのは 5-3 / 5-4 のみ
...
## 補足
- 全体の目安は3500〜3800字程度（セクション契約を満たすことを最優先し、字数は目安）
- セクション契約（最優先3施策 / 5-0 / 5-1 / 5-2）を満たせない場合は字数制限より契約遵守を優先
...
- **最優先3施策 / 5-0 予算フレーム / 5-1 LP改善 / 5-2 検索広告 は絶対に省略しないこと**
- 圧縮は Section 4 と 5-3 / 5-4 から行う（5-3 / 5-4 は `（token制約により省略）` 可）
- 表セルは短く、重複説明は禁止
