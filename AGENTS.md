...
- **ローカル開発:** Vite proxy → localhost（`vite.config.js`、`ADS_PROXY_TARGET`/`ML_PROXY_TARGET` で上書き可）
  - `/api/ml/*` → `localhost:8002` (market-lens-ai)
  - `/api/ads/*` → `localhost:8001` (ads-insights)

> ローカル/本番の実運用確認手順・`.env` のキー名（`GEMINI_API_KEY` 等の正準名）は [docs/local-verification.md](docs/local-verification.md) を参照。
...
`src/` 配下（React フロント）を変更したら、`webapp-testing` skill を使って Codex 自身が画面を開き、変更の反映と関連画面のリグレッションを確認してから完了報告する。
